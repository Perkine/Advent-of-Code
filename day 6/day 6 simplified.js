...
// reset lights
adjustLights(0, 0, 1000, 1000, 0, 0, 0);
...
function adjustLights(x0, y0, x1, y1, adjustment, min, max){
 for(var y=y0; y<y1; y++){
  for(var x=x0; x<x1; x++){
   lights[y][x] += (adjustment != 0 ? adjustment : (lights[y][x] == 0 ? 1 : -1));
   lights[y][x] = Math.min(lights[y][x], max);
   lights[y][x] = Math.max(lights[y][x], min);
  }
 }
};
...
function createProcessor(options){
 function process(value){
  ...
  args[4]++;
  args[5]++;
  args[6] = options[args[1]];
  args[7] = options.min;
  args[8] = options.max;
  args = args.slice(2);
  adjustLights.apply(null, args);
 };
 return process;
};