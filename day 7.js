...
Wire.prototype.setValue = function(value){
 ...
 wires.forEach(change, this);
 function change(wire){
  if(wire.inpA == this.name && wire.valA != this.value){
   wire.valA = this.value;
   upd = true;
  }
  if(wire.inpB == this.name && wire.valB != this.value){
   wire.valB = this.value;
   upd = true;
  }
  if(upd == true){
   wire.update();
 ...
 var operations = {
  AND: function (){
   if(this.valA == null || this.valB == null)
    return;
   this.setValue(this.valA & this.valB);
  },
  OR: function(){
   if(this.valA == null || this.valB == null)
    return;
  ...
  NOT: function(){
   if(this.valA == null && this.valB == null)
    return;
   var m = (1 << 16) - 1;
   this.setValue(m ^ (this.valA || this.valB));
  }
 };
 if(this.operation){
  operations[this.operation].call(this);
 } else if(this.valA){
  this.value = this.valA;
 }
};

function parseWireData(wire){
 var operation = wire.match(/[A-Z]+/g);
 name = wire.slice(wire.lastIndexOf(' ')+1);
 inpA = wire.match(/^[a-z]{1,2}/) || null;
 inpB = / ([a-z]{1,2}) /g.exec(wire);
 inpB = inpB && inpB[1] || null;
 valA = wire.match(/^\d+/) || null;;
 valB = wire.match(/ (\d+)/g) && wire.match(/ (\d+)/g)[0] || null;
 (operation || inpA || inpB) ? new Wire(name, inpA, valA, inpB, valB, operation) : new Wire(name, null, null, null, null, null, valA);
};

function getWire(wire){
 for(var i=0; i<wires.length; i++){
  if(wires[i].name == wire){
   return wires[i];
  }
 }
};

data.forEach(parseWireData);	// create wires

wires.forEach(function(wire){wire.setValue.call(wire)});	// init
console.log(getWire('a').value);

// overwrite wire 'b'
var index = wires.reduce(function(pos, wire, array, index){
